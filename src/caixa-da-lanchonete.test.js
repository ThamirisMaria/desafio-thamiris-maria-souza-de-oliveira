...
describe('Item', () => {
    test('setItemPrincipal em item do tipo "principal" deve resultar em "Apenas itens do tipo "extra" podem ser ligados à um item principal."', () => {
        const item = new Item("suco", "Suco Natural", 620, "principal");
        expect(item.setItemPrincipal("cafe")).toBe('Apenas itens do tipo "extra" podem ser ligados à um item principal.');
    });    
    
    test('setItemPrincipal em item do tipo "combo" deve resultar em "Apenas itens do tipo "extra" podem ser ligados à um item principal."', () => {
        const item = new Item("combo1", "1 Suco e 1 Sanduíche", 950, "combo");
        expect(item.setItemPrincipal("cafe")).toBe('Apenas itens do tipo "extra" podem ser ligados à um item principal.');
    });  
})
...
describe('CaixaDaLanchonete', () => {
    ...
    test.each([
        ...
        ['com um codigo', 'credito', 'Quantidade inválida!', ['cafe']],
        ['com um valor', 'credito', 'Item inválido!', ['1']],
        ['com código inexistente', 'debito', 'Item inválido!', ['pizza, 1']],
        ['com forma de pagamento vazia', '', 'Forma de pagamento inválida!', ['cafe, 1']],
